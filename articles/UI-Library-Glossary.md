...
This glossary provides a quick reference to the standard visual and interactive UI elements available in the built-in library. This is not an exhaustive listing. For detailed documentation, see the API reference.
...
| | [LabelButton](xref:RichHudFramework.UI.LabelButton#RichHudFramework_UI_LabelButton_examples) | A minimalist clickable text element that lacks a background texture. | Navigation tabs or low-profile actions. |
...
| | [NamedCheckBox](xref:RichHudFramework.UI.NamedCheckBox#RichHudFramework_UI_NamedCheckBox_examples) | A standard toggle control consisting of a checkbox and a descriptive label. See <xref:RichHudFramework.UI.BorderedCheckBox> for a label-free variant. | Binary (On/Off) settings where screen space is at a premium. |
| | [NamedOnOffButton](xref:RichHudFramework.UI.NamedOnOffButton#RichHudFramework_UI_NamedOnOffButton_examples) | A distinct boolean toggle featuring separate "On" and "Off" buttons. Includes a label. See <xref:RichHudFramework.UI.OnOffButton> for a label-free variant. | High-visibility settings requiring clear state indication (SE Terminal style). |
| | [NamedSliderBox](xref:RichHudFramework.UI.NamedSliderBox#RichHudFramework_UI_NamedSliderBox_examples) | A horizontal sliding control for selecting numeric values within a defined range. Includes name and value labels. See <xref:RichHudFramework.UI.SliderBox> for a label-free variant. | Continuous adjustments like volume, sensitivity, or opacity. |
| | [ScrollBar](xref:RichHudFramework.UI.ScrollBar#RichHudFramework_UI_ScrollBar_examples) | A vertical or horizontal slider used for navigation. | Scrolling text and lists. |
| | [ColorPickerHSV](xref:RichHudFramework.UI.ColorPickerHSV) | A color selection interface using Hue, Saturation, and Value sliders. | Intuitive user-facing color configuration. |
...
| | [ScrollBox](xref:RichHudFramework.UI.ScrollBox`2#RichHudFramework_UI_ScrollBox_2_examples) | A container that clips overflowing content and provides scrollbars for navigation. | Presenting long, scrolling content within a confined area. |
